...
function getRandValueOnGrid(max) {
	...
	return (
		max / (grids * 2) +
		Math.floor(Math.random() * grids) * Math.floor(max / grids)
	);
}
...
function getRandValuesOnSphere(width, height) {
	...
	if (type == 4)
		return [
			width / 2 + (Math.random() * 2 - 1) * size * 0.85,
			height / 2 + 100
		];
	if (type == 5)
		return [
	...
	if (type == 1) percentage = 0.5;
	if (type == 2) percentage = 1;
	return [
		width / 2 + Math.cos(angle) * (dist * size) * percentage,
		height / 2 + Math.sin(angle) * (dist * size)
	];
}
...
function drawDots() {
	...
	if (value < mid) newPosition = (value - start) / (mid - start);
	if (value >= mid) newPosition = 1 - (value - mid) / (end - mid);
	if (value < start) newPosition = 0;
	if (value > end) newPosition = 0;
	for (var i = 0; i < 1500; i++) {
		//console.log(value+" "+limit);
		var height1 =
			startHeight[i] + (endHeight[i] - startHeight[i]) * newPosition;
		var width1 =
			startWidth[i] + (endWidth[i] - startWidth[i]) * newPosition;
		ctx.beginPath();
...
function drawPicture() {
	...
	else size = 15 + ((value - start) / ((limit / (numPages - 1)) * 0.5)) * 100;
...
function spinBook() {
	...
	if (value <= mid)
		newValue =
			maxSpin + ((value - start) / (mid - start)) * (minSpin - maxSpin);
	else
		newValue =
			minSpin - ((value - mid) / (end - mid)) * (minSpin - maxSpin);

	console.log(newValue);
	book.style.transform = "rotateY(" + newValue + "deg)";
	if (mid - 30 < value && value < mid + 30)
		book.style.transition = "transform 0.5s ease";
	else book.style.transition = "transform 0s";
}
...
async function scrollToPage() {
	...
	console.log(
		" " +
			...
			pageSize / 4 +
			" " +
			Math.abs(scrollPosition - aux)
	);
	if (Math.abs(scrollPosition - aux) < pageSize / 2) {
		var stepSize =
			(aux - document.getElementById("container").scrollTop + 0.0) /
			steps;
...
$(function () {
	...
	document
		...
		.addEventListener("click", function () {
			var map = document.getElementById("workLocation");
			map.style.visibility =
				map.style.visibility == "visible" ? "hidden" : "visible";
		});
});
